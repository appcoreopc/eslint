...
var VALID_LEVELS = [0, 1, 2, "off", "warn", "error"];

/**
 * Checks whether all rules' level is valid or not.
 *
 * @param {object} config - A config object to check.
 * @returns {boolean} `true` if all rules' level is valid.
 */
function levelsAreValid(config) {
    return Object.keys(config).every(function(ruleId) {
        var level = config[ruleId];
...
function parseJsonConfig(string, location, messages) {
    ...
    try {
        items = options.parse(["", "", "--rule", string]).rule || {};

        // Some tests say that it should ignore invalid comments such as `/*eslint no-alert:abc*/`.
        if (levelsAreValid(items)) {
            return items;
        }
    } catch (ex) {
    ...
    api.verify = function(textOrSourceCode, config, filenameOrOptions, saveState) {
        ...
        if (ast) {
            ...
            Object.keys(config.rules).filter(function(key) {
                ...
                ruleOptions = getRuleOptions(config.rules[key]);

                try {
                    var ruleContext = new RuleContext(
                        key, api, severity, ruleOptions,
